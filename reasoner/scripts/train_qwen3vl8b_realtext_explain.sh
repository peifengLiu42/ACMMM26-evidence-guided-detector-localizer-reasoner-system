...
TRAIN_CONFIG="train_lora/qwen3vl8b_realtext_explain_lora_sft.yaml"
...
if [[ -z "$MODEL_NAME_OR_PATH" ]]; then
  echo "[reasoner-train] set MODEL_NAME_OR_PATH to your local Qwen3-VL-8B-Instruct path or model id" >&2
  exit 1
fi
...
TMP_RUN_CONFIG="$(mktemp /tmp/qwen3vl8b_realtext_explain_lora_sft.XXXXXX.yaml)"
